<script lang="ts">
	...
	const order = $derived($orderStore);
	const user = $derived($userStore);

	// Redirect if no order is found
	onMount(() => {
		if (!order) {
			// If user is logged in, redirect to orders page
			if (user) {
				goto('/orders');
			} else {
	...
	$: if (order) {
		...
		estimatedDelivery = formatEstimatedDelivery(order.createdAt, 7);
...
{:else}
	<div class="flex items-center justify-center h-[50vh]">
		<div class="text-center">
			<h1 class="text-2xl font-bold mb-4">No order information found</h1>
			<p class="text-muted-foreground mb-6">We couldn't find any order information to display.</p>
